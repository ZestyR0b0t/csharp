...
namespace SdetBootcampDay3.Exercises
{
    ...
    public class TakeHomeExercises
    {
        ...
        */
        [Test]
        public async Task SendGetUserRequest_WithValidUser_ReturnsStatusCodeOk()
        {
        ...
        [TestCase(3, "Emma")]
        [Test]
        public async Task SendGetUserRequest_WithValidUser_ReturnsAccurateName(int userId, string expectedName)
        {
            RestRequest request = new RestRequest($"/api/users/{userId}", Method.Get);
            RestResponse response = await client.ExecuteAsync(request);
            JObject responseData = JObject.Parse(response.Content!);
            string firstName = responseData.SelectToken("data.first_name")!.ToString();

            Assert.That(firstName, Is.EqualTo(expectedName));
        }
         ...
         */
        [Test]
        public async Task SendPostUserRequest_WithValidUser_ReturnsCreatedStatusCode()
        {
            TakeHomeUserDto dto = new TakeHomeUserDto
            {
                Name = "SomeName",
                Job = "SomeJob"
            };
