...
namespace SdetBootcampDay3.Exercises
{
    ...
    public class Exercises01
    {
        ...
        public async Task GetDataForUser5_CheckCompanyName_ShouldBeKeeblerLLC()
        {
            ...
            RestResponse response = await client.ExecuteAsync(request);
            JObject responseData = JObject.Parse(response.Content!);
            string companyName = responseData.SelectToken("company")!.SelectToken("name")!.ToString();

            Assert.That(companyName, Is.EqualTo("Keebler LLC"));
        }
        ...
        public async Task GetDataForUser3_CheckServerHeader_ShouldBeCloudflare()
        {
             ...
             * equal to 'cloudflare'
             */
            RestResponse response = await GetUserResponse("3"); 
            Assert.That(response.Server, Is.EqualTo("cloudflare"));
        }

        private async Task<RestResponse> GetUserResponse(string userNumber)
        {
